...
int main(){
	...
	mat4 *transforms;
	transforms = (mat4*)malloc(10000 * sizeof(mat4));

	for(int x = 0; x < 100; x++){
		for(int y = 0; y < 100; y++){
			glm_translate_to((mat4)GLM_MAT4_IDENTITY_INIT, (vec3){150 * x, 0, 150 * y}, transforms[(x * 100) + y]);
		}
	}


	struct VertexBuffer ivb;
	initVertexBuffer(&ivb, &transforms[0], 10000 * sizeof(mat4));
...
//	bindShader(shader);
	...
	//	rendererDraw(vao, ib, shader);
		rendererInstancedDraw(vao, ib, instanceShader, 10000);
